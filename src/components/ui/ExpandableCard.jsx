...
export function ExpandableCard({ cards }) {
  ...
  return (
    <>
              ...
              <div>
                <div className="flex justify-between items-start p-4">
                  ...
                  <motion.a
                    ...
                    href={active.ctaLink}
                    target="_blank"
                    className="px-4 py-3 text-sm rounded-full font-bold bg-pink-700 hover:bg-gray-900 text-white hover:text-pink-700 border border-transparent hover:border-pink-700"
                  >
                    {active.ctaText}
                  </motion.a>
                </div>
      ...
      <ul className="w-full flex flex-wrap justify-center gap-4 p-4">
        {cards.map((card, index) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={card.title}
            onClick={() => setActive(card)}
            className="p-4 flex flex-col w-[calc(100%/1)] sm:w-[calc(100%/2-1rem)] md:w-[calc(100%/3-1rem)] hover:bg-gray-800 rounded-xl cursor-pointer"
